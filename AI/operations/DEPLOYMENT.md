...
Node, Python, PostgreSQL, and Caddy do not need to be installed on the host.
...
- `caddy`: configurable `WEF_PUBLIC_PORT`, initially `3100/TCP`, after an immediate pre-deploy conflict check; TLS/standard 80/443 is a later public-launch decision.
...
- `infra/Caddyfile`: same-origin web/API/media routes and optional TLS.
...
- On interim port 3100, serves same-origin HTTP for anonymous smoke/browsing only.
- After [E7-T7](../epics/E7-production-delivery/proposed-tasks/E7-T7-enable-production-registration-and-contact-reveal.md), terminates HTTPS and redirects HTTP; authentication/contact reveal is enabled only in this mode.
- Routes `/api/*` to FastAPI and all other application routes to Next.js.
- Serves `/media/*` from the media volume mounted read-only.
- Preserves client/request IDs and correct proxy headers.
- Enables compression for text/JSON, not already compressed media.
- Adds HSTS after the domain and certificate flow are verified.
- Adds `X-Content-Type-Options: nosniff`, a conservative referrer policy, and a tested Content Security Policy.
...
Backups and restore drills are out of scope under [ADR-015](../decisions/adr/ADR-015-defer-backups.md). PostgreSQL, media, imports, Caddy state, and secrets persist on the NUC only.
...
- External HTTP uptime check for the interim 3100 endpoint; switch to HTTPS checks when [E7-T7](../epics/E7-production-delivery/proposed-tasks/E7-T7-enable-production-registration-and-contact-reveal.md) enables TLS.
- Host disk, memory, CPU, load, and Docker restart count.
- TLS expiry through Caddy/uptime service after HTTPS is enabled.
...
- Configure/forward SSH and `WEF_PUBLIC_PORT=3100`; add 80/443 only when the HTTPS design is approved.
- Configure swap only if appropriate for host memory; never use it to hide undersizing.
- Verify DNS before enabling Caddy production TLS.
