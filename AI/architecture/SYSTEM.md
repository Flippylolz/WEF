...
- Planned E19 adds a backend-mediated Groq GPT-OSS 20B boundary. `/admin/places`
  receives an owner-confirmed review for display name/address/district. A separate
  owner-started batch worker fills only missing allowlisted offer fields after
  exact-evidence, snapshot, deterministic-validator, and per-field quality gates;
  no per-offer confirmation follows the initial bounded batch authorization. The
  model has no tools or write capability. Provider calls remain outside database
  transactions; item writes are checkpointed, idempotent, and transactional.
- E19 keeps deterministic parser provenance in `OfferSource.extraction_json`, AI
  ...
  only coarse `data_origin="parser" | "ai_assisted"`; detailed provider, batch,
  source-offset, and parser-gap state remains owner-only.
