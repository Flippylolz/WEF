...
The complete locked patch/minor update produces 593,681 gzip bytes in production
JavaScript, compared with 567,719 bytes for the current application with its
previous runtime dependencies (25,962 bytes / 4.57% growth). Both measurements use
Node 22.22.2 and the existing level-9 gzip checker over every production chunk,
including lazy chunks. CI independently measured the same 593,681-byte update.

For the original #374 lockfile (593,735 bytes), controlled builds with MapLibre 6.6.0 produce 587,049 bytes; additionally restoring
...
limit is 599,617 bytes. The checker still measures all production JavaScript and
