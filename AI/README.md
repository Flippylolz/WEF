...
- Repository governance: GitHub branch protection enforces pull requests, strict CI checks, resolved conversations, linear history, and protection from force-pushes or deletion on `main`. Approving reviews are not required while the owner is the sole maintainer; the owner-only administrator bypass remains an audited emergency/bootstrap exception.
