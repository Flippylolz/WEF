...
- `user_id`.
- `visit_id`: a browser-session UUID generated by the client.
- `started_at`.
- `previous_visit_at`: nullable timestamp captured when the visit is first
  inserted.

Constraint: unique `(user_id, visit_id)`. Replays preserve both timestamps, and
runtime retention keeps the most recent 50 visits per account.
...
Constraints: unique `(user_id, offer_id)` and `view_count >= 1`. The API writes
and returns history only for offers that satisfy the public catalog visibility
policy. Both account-history tables cascade on account deletion; viewed-offer
rows also cascade when an offer is deleted.
...
- B-tree on `AccountVisit(user_id, started_at)` for bounded visit retention.
- B-tree on `ViewedOffer(user_id, last_viewed_at)` for recent self-history.
